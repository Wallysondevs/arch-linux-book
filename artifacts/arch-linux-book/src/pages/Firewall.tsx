...
export default function Firewall() {
  return (
    <PageContainer
      title="Firewall no Arch Linux"
      subtitle="Proteja seu sistema com nftables, ufw e iptables. Configure regras de entrada, saída e encaminhamento para controlar completamente o tráfego de rede."
      difficulty="avancado"
      timeToRead="20 min"
    >
      <h2>O Stack de Firewall do Linux</h2>
      <p>
        O Linux usa o <strong>netfilter</strong> no kernel para filtragem de pacotes. Existem
        várias ferramentas de espaço de usuário para gerenciar as regras:
      </p>
      <ul>
        <li><strong>iptables</strong> — Ferramenta clássica, amplamente suportada. Sendo substituída pelo nftables.</li>
        <li><strong>nftables</strong> — Substituto moderno do iptables. Mais eficiente, sintaxe melhor. Padrão em sistemas modernos.</li>
        <li><strong>ufw</strong> — "Uncomplicated Firewall". Frontend simples sobre iptables/nftables. Fácil para iniciantes.</li>
        <li><strong>firewalld</strong> — Usado no Fedora/RHEL. Suporte a zonas.</li>
      </ul>

      <h2>UFW - Para Iniciantes</h2>
      <CodeBlock
        title="UFW - Firewall simplificado"
        code={`# Instalar UFW
sudo pacman -S ufw
...
sudo ufw disable`}
      ...
      <h2>nftables - O Firewall Moderno</h2>
      <CodeBlock
        title="Conceitos do nftables"
        code={`# Instalar
...
sudo nano /etc/nftables.conf`}
      />
      <CodeBlock
        title="/etc/nftables.conf - Configuração completa"
        code={`#!/usr/bin/nft -f
...
sudo nft list ruleset > /etc/nftables.conf`}
      ...
      <h2>iptables - Compatibilidade</h2>
      <CodeBlock
        title="Comandos básicos do iptables"
...
# Rate limiting SSH
sudo iptables -A INPUT -p tcp --dport 22 -m state --state NEW -m limit \
...
sudo systemctl enable iptables`}
      ...
      <AlertBox type="info" title="nftables vs iptables">
        No Arch Linux moderno, use <strong>nftables</strong> ou <strong>ufw</strong>.
        O iptables está sendo descontinuado e o nftables é mais eficiente e poderoso.
        O ufw pode usar nftables como backend desde versões recentes.
      </AlertBox>

      <h2>Fail2ban - Proteção Automática</h2>
      <CodeBlock
