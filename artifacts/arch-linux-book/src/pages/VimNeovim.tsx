...
export default function VimNeovim() {
  return (
    <PageContainer
      title="Vim & Neovim"
      subtitle="O editor de texto mais poderoso do Linux. Do básico ao avançado — modos, comandos, personalização com Lua e transforme o Neovim em um IDE completo."
      difficulty="intermediario"
      timeToRead="25 min"
    >
      <h2>Por que aprender Vim/Neovim?</h2>
      <p>
        Vim está disponível em praticamente qualquer sistema Linux/Unix. Saber usar o Vim
        é essencial para administradores de sistemas, pois você sempre poderá editar arquivos
        mesmo em servidores sem interface gráfica ou com poucos recursos.
      </p>
      <p>
        O Neovim é um fork moderno do Vim com:
      </p>
      <ul>
        <li>Melhor suporte a plugins com Lua (mais rápido que VimScript)</li>
        <li>LSP (Language Server Protocol) nativo</li>
        <li>Tree-sitter para syntax highlighting avançado</li>
        <li>API assíncrona completa</li>
        <li>Comunidade de plugins muito ativa</li>
      </ul>

      <h2>Instalação</h2>
      <CodeBlock
        title="Instalar Vim e Neovim"
        code={`# Instalar Neovim (recomendado)
...
vim --version`}
      ...
      <h2>Os Modos do Vim</h2>
      <p>
        A característica mais distinta do Vim é ter múltiplos modos de operação:
      </p>
      <ul>
        <li><strong>Normal</strong> — Modo padrão. Para navegar e executar comandos. Tecla: <code>Esc</code></li>
        <li><strong>Insert</strong> — Para digitar texto. Tecla: <code>i</code>, <code>a</code>, <code>o</code>, etc.</li>
        <li><strong>Visual</strong> — Para selecionar texto. Tecla: <code>v</code>, <code>V</code>, <code>Ctrl+v</code></li>
        <li><strong>Command</strong> — Para executar comandos `:`. Tecla: <code>:</code></li>
        <li><strong>Replace</strong> — Para substituir caracteres. Tecla: <code>R</code></li>
      </ul>

      <h2>Comandos Essenciais de Navegação</h2>
      <CodeBlock
        title="Navegação no modo Normal"
...
%          # Pular entre parênteses/chaves/colchetes correspondentes`}
      />

      <h2>Edição de Texto</h2>
      <CodeBlock
        title="Editar no modo Normal"
        code={`# Entrar no modo INSERT
...
c3l        # Mudar (substituir) 3 caracteres à direita`}
      />

      <h2>Comandos de Arquivo</h2>
      <CodeBlock
        title="Salvar, sair e gerenciar arquivos"
        code={`# Modo de Comando (:)
:w         # Salvar
...
Ctrl+w |                # Maximizar divisão vertical`}
      />

      <h2>Busca e Substituição</h2>
      <CodeBlock
        title="Substituição avançada"
        code={`# Substituição básica (linha atual)
...
:g!/padrão/d             # Deletar linhas SEM padrão`}
      />

      <h2>Configurando o Neovim com Lua</h2>
      <CodeBlock
        title="~/.config/nvim/init.lua básico"
        code={`-- Configurações básicas
...
vim.keymap.set("v", "K", ":m '<-2<CR>gv=gv")`}
      ...
      <h2>Gerenciador de Plugins: lazy.nvim</h2>
      <CodeBlock
        title="Configurar lazy.nvim"
        code={`-- Em ~/.config/nvim/init.lua ou ~/.config/nvim/lua/plugins.lua
...
require("lazy").setup({
  ...
  { "tpope/vim-fugitive" },
      ...
      <AlertBox type="success" title="Distribuições prontas do Neovim">
        Se não quiser configurar tudo do zero, existem distribuições prontas:
        <strong>LazyVim</strong> (mais popular, baseado em lazy.nvim),
        <strong>NvChad</strong> (visual bonito) e
        <strong>AstroNvim</strong> (completo e organizado).
        Instale com um único comando e tenha um IDE completo imediatamente.
      </AlertBox>
    </PageContainer>
  );
}
