...
export default function Nodejs() {
  return (
    <PageContainer
      title="Node.js no Arch Linux"
      subtitle="Instale e gerencie Node.js com nvm e fnm, trabalhe com npm, pnpm e yarn, e configure um ambiente de desenvolvimento JavaScript/TypeScript completo."
      difficulty="iniciante"
      timeToRead="12 min"
    >
      <h2>Instalando Node.js</h2>
      <p>
        Existem três formas principais de instalar Node.js no Arch Linux, cada uma com
        seus casos de uso:
      </p>

      <h3>1. Via Pacman (versão estável)</h3>
      <CodeBlock
        title="Node.js via pacman"
        code={`# Instalar Node.js LTS do repositório
...
sudo pacman -S nodejs-current npm`}
      />

      <h3>2. Via nvm (recomendado para desenvolvedores)</h3>
      <CodeBlock
        title="Usar nvm para múltiplas versões"
        code={`# Instalar nvm
...
node --version`}
      ...
      <h3>3. Via fnm (mais rápido que nvm)</h3>
      <CodeBlock
...
echo "20" > .node-version`}
      ...
      <h2>Gerenciadores de Pacotes</h2>
      <CodeBlock
        title="npm, pnpm e yarn"
        code={`# === npm (incluído com Node.js) ===
...
ts-node src/index.ts`}
      ...
      <CodeBlock
        title="Ferramentas de desenvolvimento Node.js"
        code={`# Prettier - Formatação de código
...
npm audit fix`}
      ...
      <AlertBox type="info" title="Node.js com processos do sistema">
        Para scripts Node.js que precisam de acesso ao sistema (como ouvir na porta 80),
        não rode como root. Em vez disso, use <code>authbind</code> ou configure redirecionamento
        de porta com iptables/nftables: <code>iptables -t nat -A PREROUTING -p tcp --dport 80 -j REDIRECT --to-port 3000</code>
      </AlertBox>
    </PageContainer>
  );
}
