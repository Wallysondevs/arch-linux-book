...
export default function Automacao() {
  return (
    <PageContainer
      title="Automação com Cron & systemd"
      subtitle="Automatize tarefas repetitivas com cron, systemd timers e shell scripts. Backups automáticos, alertas, manutenção e muito mais."
      difficulty="intermediario"
      timeToRead="15 min"
    >
      <h2>Cron vs systemd Timers</h2>
      <ul>
        <li><strong>Cron</strong> — Clássico, simples, funciona bem. Limitações: sem dependências, logs básicos, não sabe se o sistema estava desligado.</li>
        <li><strong>systemd timers</strong> — Moderno, integrado ao systemd. Logs no journald, suporte a Persistent (roda quando o sistema liga), dependências, e mais controle.</li>
      </ul>

      <h2>Cron</h2>
      <CodeBlock
        title="Configurar tarefas cron"
        code={`# Instalar cron (Arch não vem com cron)
...
# 1,3,5 = 1, 3 e 5`}
      />
      <CodeBlock
        title="Exemplos de crontabs"
        code={`# Executar backup todos os dias às 2h da manhã
...
@daily /usr/local/bin/relatorio-diario.sh`}
      />

      <h2>systemd Timers</h2>
      <CodeBlock
        title="Criar um systemd timer"
        code={`# Exemplo: Timer para verificar atualizações diariamente

# 1. Criar o service (o que vai ser executado)
sudo nano /etc/systemd/system/verificar-updates.service`}
      />
      <CodeBlock
        title="verificar-updates.service"
        code={`[Unit]
...
WantedBy=timers.target`}
      />
      <CodeBlock
        title="Ativar e gerenciar timers"
        code={`# Recarregar configurações
...
# Ver logs do service
journalctl -u verificar-updates.service -n 50`}
      />

      <h2>Shell Scripts de Automação</h2>
      <CodeBlock
        title="Script de monitoramento do sistema"
        code={`#!/bin/bash
...
if [ "$DISCO" -gt "$ALERTA_DISCO" ]; then
    ...
    journalctl --vacuum-size=500M    # Limpar logs antigos do journald
      ...
      <h2>at - Executar uma Vez no Futuro</h2>
      <CodeBlock
        title="Agendar tarefas únicas com at"
        code={`# Instalar at
sudo pacman -S at
...
# Ctrl+D para finalizar`}
      ...
      <h2>Notificações de Desktop</h2>
      <CodeBlock
        title="Notificações automáticas no desktop"
        code={`# Instalar libnotify
...
# O service rodando como usuário já tem acesso ao DISPLAY`}
      />
    </PageContainer>
  );
}
