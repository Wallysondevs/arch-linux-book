...
export default function Fontes() {
  return (
    <PageContainer
      title="Fontes & Renderização"
      subtitle="Configure fontes, renderização de texto, hinting, antialiasing e emoji no Arch Linux para uma experiência visual perfeita."
      difficulty="iniciante"
      timeToRead="15 min"
    >
      <h2>Fontes Essenciais</h2>
      <p>
        O Arch Linux vem com poucas fontes instaladas por padrão. Para uma experiência visual adequada,
        você precisa instalar fontes para texto, código, emojis e caracteres internacionais.
      </p>

      <CodeBlock
        title="Instalar fontes essenciais"
        code={`# Fontes básicas de sistema
sudo pacman -S \\
  noto-fonts \\
  noto-fonts-cjk \\
  noto-fonts-emoji \\
...
sudo pacman -S ttf-font-awesome otf-font-awesome`}
      ...
      <AlertBox type="info" title="Fontes Nerd">
        As Nerd Fonts são fontes para programação com ícones embutidos. São essenciais para
        terminais modernos (Starship, Powerlevel10k) e editores como Neovim com plugins de ícones.
        O sufixo <code>-nerd</code> no nome do pacote indica que é uma Nerd Font.
      </AlertBox>

      <h2>Configuração de Renderização (Fontconfig)</h2>
      <p>
        O <code>fontconfig</code> controla como as fontes são renderizadas — antialiasing, hinting,
        substituição de fontes e ordem de preferência.
      </p>

      <CodeBlock
        title="Configuração pessoal de fontes"
...
<fontconfig>
  ...
  <alias>
    ...
    <prefer><family>JetBrainsMono Nerd Font</family></prefer>
      ...
      <CodeBlock
        title="Gerenciar fontes com fc-* tools"
        code={`# Listar todas as fontes instaladas
fc-list
...
fc-query /usr/share/fonts/TTF/JetBrainsMonoNerdFont-Regular.ttf`}
      ...
      <h2>Instalar Fontes Manualmente</h2>

      <CodeBlock
        title="Instalar fontes baixadas manualmente"
        code={`# Fontes para todos os usuários
...
fc-cache -fv`}
      ...
      <h2>Configuração de Emojis</h2>

      <CodeBlock
        title="Configurar emojis corretamente"
        code={`# Instalar fonte de emoji
sudo pacman -S noto-fonts-emoji
...
echo "🎉 🚀 ❤️ 🔥 ⭐ 🎮 💻 🐧"`}
      ...
      <h2>Fontes por Aplicação</h2>

      <CodeBlock
        title="Configurar fontes em aplicações específicas"
        code={`# Terminal (Alacritty) — ~/.config/alacritty/alacritty.toml
[font]
size = 12.0
[font.normal]
...
sudo pacman -S qt5ct`}
      ...
      <h2>DPI e Escala HiDPI</h2>

      <CodeBlock
        title="Configurar HiDPI (telas de alta resolução)"
        code={`# GNOME — suporte nativo
...
export QT_SCALE_FACTOR=2`}
      ...
      <CodeBlock
        title="Resolver problemas comuns de fontes"
        code={`# Fontes feias ou serrilhadas
