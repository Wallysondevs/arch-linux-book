...
export default function Cgroups() {
  return (
    <PageContainer
      ...
      subtitle="As bases de containers: entenda como o Linux isola processos, limita recursos e cria ambientes separados."
      difficulty="avancado"
      timeToRead="18 min"
    >
      <h2>O que são Cgroups?</h2>
      <p>
        <strong>Control Groups (cgroups)</strong> são um mecanismo do kernel Linux para limitar, priorizar 
        e monitorar recursos (CPU, memória, I/O, rede) de grupos de processos. Eles são a base tecnológica 
        que torna containers (Docker, Podman) possíveis.
      </p>

      <h2>Cgroups v2</h2>

      <CodeBlock
        title="Verificar e usar cgroups"
        code={`# Verificar se cgroups v2 está ativo (padrão em kernels modernos)
mount | grep cgroup2
# ou
...
cat /sys/fs/cgroup/memory.stat`}
      ...
      <h2>Limitando Recursos com systemd</h2>
      <p>
        O systemd é a interface mais prática para usar cgroups. Cada serviço roda em seu próprio cgroup.
      </p>

      <CodeBlock
        title="Limitar recursos de serviços"
        code={`# Ver cgroups de um serviço
...
# Os limites persistem em /etc/systemd/system/nginx.service.d/`}
      />

      <CodeBlock
        title="Limitar recursos de um comando específico"
        code={`# Executar com limite de recursos usando systemd-run
...
systemd-run --scope --unit=minha-compilacao -p CPUQuota=200% -p MemoryMax=8G make`}
      />

      <h2>Namespaces</h2>
      <p>
        <strong>Namespaces</strong> isolam diferentes aspectos do sistema para um processo, fazendo ele "pensar"
        que tem seu próprio sistema operacional. São 8 tipos de namespaces no Linux:
      </p>

      <CodeBlock
        title="Tipos de namespaces"
        code={`# PID    — Processos isolados (PID 1 próprio)
...
lsns`}
      ...
      <CodeBlock
        title="Criar namespaces manualmente"
        code={`# Criar namespace de rede isolado
...
# -R = set root directory`}
      ...
      <h2>Monitorando Cgroups</h2>

      <CodeBlock
        title="Monitorar uso de recursos por cgroup"
        code={`# Ver árvore de cgroups com systemd
systemd-cgls
...
cat /sys/fs/cgroup/system.slice/nginx.service/io.stat`}
      ...
      <h2>Cgroups para Usuários</h2>

      <CodeBlock
        title="Limitar recursos por usuário"
        code={`# O systemd cria um cgroup por sessão de usuário
...
systemd-cgtop -m`}
      ...
      <AlertBox type="info" title="Cgroups na prática">
        No dia a dia, você raramente manipula cgroups diretamente. Docker, Podman, systemd e até 
        navegadores web (Chrome) usam cgroups internamente para isolamento e controle de recursos. 
        Entender cgroups ajuda a diagnosticar problemas de performance e segurança.
      </AlertBox>
    </PageContainer>
  );
}
